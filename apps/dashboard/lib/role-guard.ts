...
export async function resolveCallerRole(
  ...
  if (userRecord?.tenantId !== tenantId && role !== 'SUPER_ADMIN') return null;
...
export async function requireSuperAdmin(): Promise<{
  ...
  // Check platform-level User.role first
  if (normaliseRole(user.role) === 'SUPER_ADMIN') return null;

  // Then check if they hold SUPER_ADMIN in ANY workspace (platform operator)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const dbClient = prisma as any;
  if (typeof dbClient.workspaceMember?.findFirst === 'function') {
    ...
    if (superMembership) return null;
  }
