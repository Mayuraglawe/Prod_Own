...
export default NextAuth(authConfig).auth((req) => {
  try {
    ...
    const user = req.auth?.user as
      { role?: string; tenantId?: string } | undefined;
    ...
    if (isLoggedIn) {
      if (
        ...
        return Response.redirect(
          new URL(`/${rolePrefix}/dashboard`, req.nextUrl),
        );
      }

      // Protect role-specific routes (e.g. employee cannot access /admin)
      const accessingRolePath = rolePaths.find((p) =>
    ...
    return new NextResponse("Internal Server Error in Middleware", {
      status: 500,
    });
  }
});
