...
export default NextAuth(authConfig).auth((req) => {
  try {
    ...
    if (isLoggedIn) {
      ...
      if (isAuthRoute || req.nextUrl.pathname === "/" || req.nextUrl.pathname === "/dashboard") {
        const targetPath = tenantId ? `/${rolePrefix}/dashboard` : `/${rolePrefix}/onboarding`;
        return Response.redirect(new URL(targetPath, req.nextUrl));
      }
      ...
      // Protect dashboard routes for users without a tenant
      if (req.nextUrl.pathname.includes("/dashboard") && !tenantId) {
        return Response.redirect(new URL(`/${rolePrefix}/onboarding`, req.nextUrl));
      }

      // Redirect users who already have a tenant away from the onboarding page
      if (req.nextUrl.pathname.includes("/onboarding") && tenantId) {
