...
export async function createWorkspace(formData: FormData) {
  ...
  // Force NextAuth to update the session cookie with the new tenantId
  // so the middleware doesn't bounce them back to the onboarding page.
  // @ts-expect-error - NextAuth types don't natively include tenantId yet
  await unstable_update({ user: { tenantId: newTenantId } });
